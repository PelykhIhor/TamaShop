...
const contactInfo = {
    shop: "Найкращий мазагин сучасної техніки",
...
function ContactBar() {
    return (
        <nav className="flex px-5 py-3 bg-white border-gray-200 dark:bg-gray-900 m-5 rounded-xl shadow" aria-label="Breadcrumb">
    ...
    return (
        <div className="flex-1 flex items-center justify-center">
            <a className="text-purple-700 hover:text-pink-600 flex items-center">
                <span className="mr-4">{label === 'phoneNumber' ? `Телефон: ${value}` : value}</span>
            </a>
        </div>
    );
}
...
function NavItem({ href, text }) {
    return (
        <li>
            <a href={href} className="block py-2 px-3 text-2xl rounded md:bg-transparent text-purple-700 hover:text-pink-600 md:p-0 dark:text-white md:dark:text-purple-500">
                {text}
            </a>
        </li>
    );
